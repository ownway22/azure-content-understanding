...
KEY = os.getenv("KEY")
IMG_ANALYZER_ID = os.getenv("IMG_ANALYZER_ID")
IMG_FILE_URL = os.getenv("IMG_FILE_URL")

def analyze_image():
    """傳送圖片進行分析"""
    url = f"{ENDPOINT}contentunderstanding/analyzers/{IMG_ANALYZER_ID}:analyze?api-version=2025-05-01-preview"
    ...
    print("=" * 60)
    print("Step 1: Sending image for analysis...")
    print(f"Endpoint: {ENDPOINT}")
    print(f"Analyzer ID: {IMG_ANALYZER_ID}")
    print(f"File URL: {IMG_FILE_URL}")
    print("=" * 60)
...
def save_result(result):
    ...
    filename = f"img_result_{date_stamp}_{time_stamp}.json"
...
def main():
    print("\n🖼️ Azure Content Understanding - Image Analysis")
    print("=" * 60)
    
    # 步驟 1: 分析圖片
    request_id = analyze_image()
