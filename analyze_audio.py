...
KEY = os.getenv("KEY")
AUDIO_ANALYZER_ID = os.getenv("AUDIO_ANALYZER_ID")
AUDIO_FILE_URL = os.getenv("AUDIO_FILE_URL")

def analyze_audio():
    """傳送音訊進行分析"""
    url = f"{ENDPOINT}contentunderstanding/analyzers/{AUDIO_ANALYZER_ID}:analyze?api-version=2025-05-01-preview"
    ...
    print("=" * 60)
    print("Step 1: Sending audio for analysis...")
    print(f"Endpoint: {ENDPOINT}")
    print(f"Analyzer ID: {AUDIO_ANALYZER_ID}")
    print(f"File URL: {AUDIO_FILE_URL}")
    print("=" * 60)
...
def save_result(result):
    ...
    filename = f"audio_result_{date_stamp}_{time_stamp}.json"
...
def main():
    print("\n🎵 Azure Content Understanding - Audio Analysis")
    print("=" * 60)
    
    # 步驟 1: 分析音訊
    request_id = analyze_audio()
    
    if not request_id:
        print("\n❌ Failed to submit audio for analysis")
        return
