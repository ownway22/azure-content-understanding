...
KEY = os.getenv("KEY")
DOC_ANALYZER_ID = os.getenv("DOC_ANALYZER_ID")
DOC_FILE_URL = os.getenv("DOC_FILE_URL")

def analyze_document():
    """傳送檔案進行分析"""
    url = f"{ENDPOINT}contentunderstanding/analyzers/{DOC_ANALYZER_ID}:analyze?api-version=2025-05-01-preview"
    ...
    print("=" * 60)
    print("Step 1: Sending document for analysis...")
    print(f"Endpoint: {ENDPOINT}")
    print(f"Analyzer ID: {DOC_ANALYZER_ID}")
    print(f"File URL: {DOC_FILE_URL}")
    print("=" * 60)
...
def save_result(result):
    ...
    filename = f"doc_result_{date_stamp}_{time_stamp}.json"
...
def main():
    print("\n🚀 Azure Content Understanding - Document Analysis")
    print("=" * 60)
    
    # 步驟 1: 分析文件
    request_id = analyze_document()
    
    if not request_id:
        print("\n❌ Failed to submit document for analysis")
        return
